...
func CreateDirectoryService(userEmail string) (*admin.Service, error) {
  ...
  jsonCredentials, err := ioutil.ReadFile(Cfg.Credentials)
  if err != nil {
    return nil, err
  }
...
func PullGsuiteUsers(email string, domain string, mask string) ([]IAMUser, error) {

	// gsuiteUsers List of gsuiteUser objects
	var gsuiteUsers = []IAMUser {}

	srv, e := CreateDirectoryService(email)
	if e != nil {
		return nil, e
	}
